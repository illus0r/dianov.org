...
На сайте Банков.ру есть таблица с народным рейтингом. Надо показать её данные так, чтобы от них была польза. Мне было бы интересно узнать, где открыть депозит и куда вкладывать деньги, поэтому в качестве целевой аудитории выбрал физических лиц.
...
Ограничусь высшей лигой, то есть пятьюдесятью банками с наибольшим количеством зачтенных отзывов за последние 365 дней. Перебираю все недели с 2005 по 2017 гг., для каждой скачиваю `.json`. Склеиваю в один файл:
...
Хочется ещё проверить зависимость рейтинга и средней оценки от времени. Делаю черновую диаграму из червяков. Видна линейная зависимость между величинами, но есть и отклонения. Например, рейтинг  Совкомбанка в 2015 году растёт, хотя средняя оценка не меняется:
...
При написании отзыва на Банках.ру люди выбрают от 0 до 12 продуктов
{:.caption}
...
О, как! Не задумывался о таком. Значит, резкие приросты отзывов — не обязательно читерство. Здорово бы показать на графике, как банки разъединяются и сливаются, но в эту итерацию не успею.
...
Сотня графиков — слишком много. Они скачут и путаются, выходит неразбериха. Надо показать только некоторые, но тогда пропадёт полнота картини. Проблему решает «скользящее окно»: видны все графики сразу, жирным выделены лишь те, превьюшки которых видны на экране. Показал паре испытуемых, они 
...
Если бы не дедлайн, вот что я хотел бы сделать:

* Заменить случайные значения оценок продуктов на настоящие. Я [нашёл, где их взять](http://www.banki.ru/services/responses/list/).
* Проверить правильность рейтинга Банков.ру, пересчитав оценки банков в рейтинг по их формуле. Сравнить графики.
* Сделать рейтинги по продуктам, добавить переключалку для выбора нужного продкута.
...
* Добавить возможнсть сравнить банки. Кликаю банк — он залипает и остаётся чёрным, даже когда я убераю с него мышку. По большому графику вслед за курсором двигаестся бегунок с датой, оценкой и числом отзывов. В превьюшках при этом обновляются оценки и синхронно двигаются маленькие бегунки.
* Сделать макет адаптивным, сейчас только на макбуке красиво смотрится.
* Показать спиннер, пока данные грузятся.
